...
def download_game(game, process, path_to_raw, path_to_final):
    ...
    if process == True:
        try:
            processed_data = sdv.wnba.wnba_pbp_disk(game_id=game, path_to_json=path_to_raw)

            result = sdv.wnba.helper_wnba_pbp(game_id=game, pbp_txt=processed_data)
...
def add_game_to_schedule(schedule, year):
    ...
    schedule["game_json_url"] = np.where(
        ...
        schedule["game_id"].apply(
            lambda x: f"https://raw.githubusercontent.com/sportsdataverse/wehoop-wnba-raw/main/wnba/json/final/{x}.json"
        ),
        None,
    )
    schedule.to_parquet(f"wnba/schedules/parquet/wnba_schedule_{year}.parquet", index=None)
    pyreadr.write_rds(f"wnba/schedules/rds/wnba_schedule_{year}.rds", schedule, compress="gzip")
    return
...
def main():
    ...
    for year in years_arr:
        schedule = pd.read_parquet(f"wnba/schedules/parquet/wnba_schedule_{year}.parquet", engine="auto", columns=None)
        ...
        logger.info(f"Scraping WNBA PBP for {year}...")
        ...
        logger.info(f"Finished WNBA PBP for {year}...")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Start year of WNBA Schedule period (YYYY), eg. 2023 for 2022-23 season",
    )
    parser.add_argument(
        "--end_year", "-e", type=int, help="End year of WNBA Schedule period (YYYY), eg. 2023 for 2022-23 season"
    )
    parser.add_argument("--rescrape", "-r", type=str2bool, default=True, help="Rescrape all games in the schedule period")
    parser.add_argument(
