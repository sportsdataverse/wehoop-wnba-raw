...
the per-game tidy officials frame (one row per official-game).
...
logging.basicConfig(
    level=logging.INFO,
    filename="wehoop_wnba_raw_officials_logfile.txt",
)
logger = logging.getLogger(__name__)

PATH_TO_OUTPUT = "wnba/officials/json"
...
def download_officials(game_id: int, output_dir: Path, rerun_existing: bool) -> str:
    ...
    try:
        ...
        # schema drift, and transient ESPN failures must NOT abort the run.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Start year of WNBA officials scrape (YYYY), e.g. 2025",
    )
    parser.add_argument(
        "--end_year",
        "--end-year",
        ...
        dest="end_year",
        type=int,
        help="End year of WNBA officials scrape (YYYY)",
    )
    parser.add_argument(
        "--cores",
        "--workers",
    ...
    parser.add_argument(
        ...
        help="Re-scrape officials even when the output file already exists. Accepts a true/false value (e.g. `-r true`) for compat with the legacy umbrella workflow; bare `-r` defaults to True.",
    )
    parser.add_argument(
        "--force",
        action="store_true",
