...
def add_game_to_schedule(schedule, year):
    ...
    schedule["game_json_url"] = np.where(
        ...
        schedule["game_id"].apply(
            lambda x: (
                f"https://raw.githubusercontent.com/sportsdataverse/wehoop-wnba-raw/main/wnba/json/final/{x}.json"
            )
        ),
        None,
    )
...
def main():
    ...
    for year in years_arr:
        schedule = pd.read_parquet(
            f"wnba/schedules/parquet/wnba_schedule_{year}.parquet", engine="auto", columns=None
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "-e",
        type=int,
        help="End year of WNBA Schedule period (YYYY), eg. 2023 for 2022-23 season",
    )
    parser.add_argument(
        "--rescrape",
        "-r",
